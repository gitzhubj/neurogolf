...
- 输入尺寸: 4 unique sizes，输出尺寸: 3x3 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 6, 8]，输出颜色: [0, 2, 3, 4, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task134.json`。
- 架构方案已由 baseline ONNX 验证通过（68 节点，7220 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task134.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 68 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Add+And+ArgMax+Cast+Div+Equal+Gather+Greater+Less+MaxPool+Mul+Pad+ReduceMax+Reshape+Slice+Sub+Sum+Where (68 nodes, 25 initializers, 7220 bytes)
...
baseline_ops: Add+And+ArgMax+Cast+Div+Equal+Gather+Greater+Less+MaxPool+Mul+Pad+ReduceMax+Reshape+Slice+Sub+Sum+Where
baseline_nodes: 68
baseline_size_bytes: 7220
```
