...
- 核心变换：形状镜像：浅蓝(8)形状水平镜像至黄色(4)T形不对称侧，填充空白区域。
...
- `fusion_hint`: Gather + supporting ops. 21 nodes total.
- `approach`: Use Gather-based lookup or spatial permutation — no Conv needed.

Baseline 实际架构: **Gather-based multi-op** — And+Concat+Equal+Gather+Greater+Or+Pad+Slice+Where (21 nodes, 21 initializers, 3083 bytes)
...
baseline_ops: And+Concat+Equal+Gather+Greater+Or+Pad+Slice+Where
baseline_nodes: 21
baseline_size_bytes: 3083
```
