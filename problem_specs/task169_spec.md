...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task169.json`。
- 架构方案已由 baseline ONNX 验证通过（179 节点，23816 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task169.json`。
...
- `fusion_hint`: Study baseline: 179 nodes, ops=And+Cast+Concat+Conv+Equal+Gather+Greater+Pad+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — And+Cast+Concat+Conv+Equal+Gather+Greater+Pad+Sub+Sum (179 nodes, 71 initializers, 23816 bytes)
...
baseline_ops: And+Cast+Concat+Conv+Equal+Gather+Greater+Pad+Sub+Sum
baseline_nodes: 179
baseline_size_bytes: 23816
```
