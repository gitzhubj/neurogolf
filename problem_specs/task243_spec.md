...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task243.json`。
- 架构方案已由 baseline ONNX 验证通过（437 节点，23604 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task243.json`。
...
- `fusion_hint`: Study baseline: 437 nodes, ops=Add+Cast+Concat+Conv+Greater+Mul+Slice+Sub
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Add+Cast+Concat+Conv+Greater+Mul+Slice+Sub (437 nodes, 24 initializers, 23604 bytes)
...
baseline_ops: Add+Cast+Concat+Conv+Greater+Mul+Slice+Sub
baseline_nodes: 437
baseline_size_bytes: 23604
```
