...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 2, 4, 5, 6, 8]，输出颜色: [0, 1, 2, 4, 6, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task368.json`。
- 架构方案已由 baseline ONNX 验证通过（183 节点，14770 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task368.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 183 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Abs+ArgMax+Cast+Concat+Less+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where (183 nodes, 32 initializers, 14770 bytes)
...
arcgen_samples: 262
same_size: True
colors_in: [0, 1, 2, 4, 5, 6, 8]
colors_out: [0, 1, 2, 4, 6, 8]
locality: global
single_linear_conv_possible: no
recommended_architecture: reduce_with_where
recommended_kernel: not_needed
...
baseline_ops: Abs+ArgMax+Cast+Concat+Less+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 183
baseline_size_bytes: 14770
```
