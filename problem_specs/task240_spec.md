...
- 输入尺寸: 19x19 (all same)，输出尺寸: 19x19 (all same)。
- 同尺寸变换: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task240.json`。
- 架构方案已由 baseline ONNX 验证通过（28 节点，3953 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task240.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 28 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Concat+Max+MaxPool+Mul+Pad+ReduceSum+Slice+Sub (28 nodes, 20 initializers, 3953 bytes)
...
baseline_ops: Concat+Max+MaxPool+Mul+Pad+ReduceSum+Slice+Sub
baseline_nodes: 28
baseline_size_bytes: 3953
```
