...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task356.json`。
- 架构方案已由 baseline ONNX 验证通过（21 节点，2344 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task356.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 21 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — And+Concat+CumSum+Greater+Not+Or+Pad+ReduceSum+Slice+Sub (21 nodes, 10 initializers, 2344 bytes)
...
baseline_ops: And+Concat+CumSum+Greater+Not+Or+Pad+ReduceSum+Slice+Sub
baseline_nodes: 21
