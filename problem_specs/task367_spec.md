...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task367.json`。
- 架构方案已由 baseline ONNX 验证通过（694 节点，293653 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task367.json`。
...
- `fusion_hint`: Study baseline: 694 nodes, ops=Add+Cast+Concat+Conv+ConvTranspose+Greater+Mul+ReduceSum+Slice+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Add+Cast+Concat+Conv+ConvTranspose+Greater+Mul+ReduceSum+Slice+Sub+Sum (694 nodes, 474 initializers, 293653 bytes)
...
baseline_ops: Add+Cast+Concat+Conv+ConvTranspose+Greater+Mul+ReduceSum+Slice+Sub+Sum
baseline_nodes: 694
baseline_size_bytes: 293653
```
