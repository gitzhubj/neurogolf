...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task330.json`。
- 架构方案已由 baseline ONNX 验证通过（484 节点，43441 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task330.json`。
...
- `fusion_hint`: Study baseline: 484 nodes, ops=And+Cast+Concat+Conv+Gather+Greater+Less+Mul+Pad+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — And+Cast+Concat+Conv+Gather+Greater+Less+Mul+Pad+Sub+Sum+Where (484 nodes, 136 initializers, 43441 bytes)
...
baseline_ops: And+Cast+Concat+Conv+Gather+Greater+Less+Mul+Pad+Sub+Sum+Where
baseline_nodes: 484
baseline_size_bytes: 43441
```
