...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task114.json`。
- 架构方案已由 baseline ONNX 验证通过（26 节点，4719 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task114.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — ArgMax+Cast+Concat+Equal+GatherElements+Not+Pad+ReduceMax+Reshape+Slice+Where (26 nodes, 21 initializers, 4719 bytes)
...
baseline_ops: ArgMax+Cast+Concat+Equal+GatherElements+Not+Pad+ReduceMax+Reshape+Slice+Where
baseline_nodes: 26
baseline_size_bytes: 4719
```
