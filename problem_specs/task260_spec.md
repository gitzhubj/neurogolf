...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 2, 5, 7, 9]，输出颜色: [0, 1, 2, 7, 9]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task260.json`。
- 架构方案已由 baseline ONNX 验证通过（26 节点，3320 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task260.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — And+Equal+Greater+Less+Mul+Or+Pad+ReduceMax+ReduceMin+Slice+Sub+Sum+Where (26 nodes, 15 initializers, 3320 bytes)
...
arcgen_samples: 262
same_size: True
colors_in: [0, 1, 2, 5, 7, 9]
colors_out: [0, 1, 2, 7, 9]
locality: global
single_linear_conv_possible: no
recommended_architecture: reduce_with_where
recommended_kernel: not_needed
...
baseline_ops: And+Equal+Greater+Less+Mul+Or+Pad+ReduceMax+ReduceMin+Slice+Sub+Sum+Where
baseline_nodes: 26
baseline_size_bytes: 3320
```
