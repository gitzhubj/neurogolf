...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task390.json`。
- 架构方案已由 baseline ONNX 验证通过（41 节点，7851 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task390.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — Add+And+ArgMax+Concat+Gather+Greater+Less+Mul+ReduceMax+Reshape+Slice+Sub+Where (41 nodes, 11 initializers, 7851 bytes)
...
baseline_ops: Add+And+ArgMax+Concat+Gather+Greater+Less+Mul+ReduceMax+Reshape+Slice+Sub+Where
baseline_nodes: 41
baseline_size_bytes: 7851
```
