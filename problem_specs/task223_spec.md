...
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 6, 7]，输出颜色: [0, 1, 2, 3, 4, 6, 7]。
- 颜色集一致: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task223.json`。
- 架构方案已由 baseline ONNX 验证通过（1 节点，7454 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task223.json`。
