...
- 核心变换：最大蓝点分量：9x9中4个3x3连通分量，选取蓝色(1)像素最多的分量输出。
...
- `fusion_hint`: Study baseline: 22 nodes, ops=ArgMax+Conv+Div+Gather+Greater+Mul+Pad+Reshape+Squeeze+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — ArgMax+Conv+Div+Gather+Greater+Mul+Pad+Reshape+Squeeze+Sub+Sum+Where (22 nodes, 10 initializers, 1993 bytes)
...
baseline_ops: ArgMax+Conv+Div+Gather+Greater+Mul+Pad+Reshape+Squeeze+Sub+Sum+Where
baseline_nodes: 22
baseline_size_bytes: 1993
```
