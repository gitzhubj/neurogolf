...
- 输入尺寸: 9x4 (all same)，输出尺寸: 4x4 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 2, 4, 7]，输出颜色: [0, 3]。
- 颜色集一致: 否。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task144.json`。
- 架构方案已由 baseline ONNX 验证通过（11 节点，1639 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task144.json`。
...
- `fusion_hint`: Ops: Concat+Mul+Pad+Slice+Sub+Sum...
- `approach`: Study baseline ONNX directly for optimal architecture.

Baseline 实际架构: **Custom multi-op** — Concat+Mul+Pad+Slice+Sub+Sum (11 nodes, 8 initializers, 1639 bytes)
