...
- 输入尺寸: 3x6 (all same)，输出尺寸: 3x3 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 3, 4]，输出颜色: [0, 6]。
- 颜色集一致: 否。
...
- 训练样本: 5 个，测试样本: 2 个，ARC-GEN 样本: 262 个。
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task347.json`。
- 架构方案已由 baseline ONNX 验证通过（10 节点，1369 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task347.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Max+Mul+Pad+ReduceMax+Slice+Sub+Sum (10 nodes, 10 initializers, 1369 bytes)
...
baseline_ops: Max+Mul+Pad+ReduceMax+Slice+Sub+Sum
baseline_nodes: 10
baseline_size_bytes: 1369
```
