...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task250.json`。
- 架构方案已由 baseline ONNX 验证通过（127 节点，13543 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task250.json`。
...
- `fusion_hint`: Study baseline: 127 nodes, ops=And+Cast+Conv+Equal+Greater+Less+Mul+Or+Pad+ReduceSum+Slice+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — And+Cast+Conv+Equal+Greater+Less+Mul+Or+Pad+ReduceSum+Slice+Sub+Sum+Where (127 nodes, 19 initializers, 13543 bytes)
...
baseline_ops: And+Cast+Conv+Equal+Greater+Less+Mul+Or+Pad+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 127
baseline_size_bytes: 13543
```
