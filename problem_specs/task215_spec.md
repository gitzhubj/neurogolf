...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task215.json`。
- 架构方案已由 baseline ONNX 验证通过（38 节点，4015 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task215.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 38 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Cast+Equal+Greater+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where (38 nodes, 19 initializers, 4015 bytes)
...
baseline_ops: Cast+Equal+Greater+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 38
baseline_size_bytes: 4015
```
