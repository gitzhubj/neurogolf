...
- 输入尺寸: 3 unique sizes，输出尺寸: 9x9 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task264.json`。
- 架构方案已由 baseline ONNX 验证通过（115 节点，10074 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task264.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 115 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Cast+Gather+Mul+Pad+ReduceSum+Slice+Sub+Sum (115 nodes, 38 initializers, 10074 bytes)
...
baseline_ops: Cast+Gather+Mul+Pad+ReduceSum+Slice+Sub+Sum
baseline_nodes: 115
baseline_size_bytes: 10074
```
