...
- 输入尺寸: 8x8 (all same)，输出尺寸: 8x8 (all same)。
- 同尺寸变换: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task314.json`。
- 架构方案已由 baseline ONNX 验证通过（218 节点，28414 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task314.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 218 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Concat+Mul+Pad+ReduceSum+Slice+Sub+Sum (218 nodes, 83 initializers, 28414 bytes)
...
baseline_ops: Concat+Mul+Pad+ReduceSum+Slice+Sub+Sum
baseline_nodes: 218
baseline_size_bytes: 28414
```
