...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task128.json`。
- 架构方案已由 baseline ONNX 验证通过（28 节点，3365 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task128.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — Add+ArgMax+Cast+Concat+Equal+Gather+Less+MatMul+Mul+ReduceMax+ReduceSum+Reshape+Slice+Sub+Where (28 nodes, 13 initializers, 3365 bytes)
...
baseline_ops: Add+ArgMax+Cast+Concat+Equal+Gather+Less+MatMul+Mul+ReduceMax+ReduceSum+Reshape+Slice+Sub+Where
baseline_nodes: 28
baseline_size_bytes: 3365
```
