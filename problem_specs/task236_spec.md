...
- 核心变换：上下半XOR：上半蓝(1)和下半红(2)逐像素XOR，结果为绿色(3)，无结果清零。
...
- `fusion_hint`: Slice + supporting ops. 9 nodes total.
- `approach`: Use Slice+Pad for spatial crop/flip/reposition — no Conv needed.

Baseline 实际架构: **Slice-based multi-op** — Abs+Concat+Pad+Slice+Sub (9 nodes, 8 initializers, 1511 bytes)
...
baseline_ops: Abs+Concat+Pad+Slice+Sub
baseline_nodes: 9
baseline_size_bytes: 1511
```
