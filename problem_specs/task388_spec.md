...
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 2, 3, 4, 5]，输出颜色: [0, 2, 3, 4, 5, 8]。
- 颜色集一致: 否。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task388.json`。
- 架构方案已由 baseline ONNX 验证通过（48 节点，6368 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task388.json`。
...
- `fusion_hint`: Study baseline: 48 nodes, ops=Conv+Equal+Mul+Pad+ReduceMax+ReduceSum+Slice+Sqrt+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Conv+Equal+Mul+Pad+ReduceMax+ReduceSum+Slice+Sqrt+Sub+Sum+Where (48 nodes, 30 initializers, 6368 bytes)
...
baseline_ops: Conv+Equal+Mul+Pad+ReduceMax+ReduceSum+Slice+Sqrt+Sub+Sum+Where
baseline_nodes: 48
baseline_size_bytes: 6368
```
