...
- 同尺寸变换: 是。
- 输入颜色: [0, 2, 3, 4, 6, 8]，输出颜色: [0, 2, 3, 4, 5, 6, 8]。
- 颜色集一致: 否。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task232.json`。
- 架构方案已由 baseline ONNX 验证通过（27 节点，2435 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task232.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 27 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Concat+Greater+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where (27 nodes, 14 initializers, 2435 bytes)
...
baseline_ops: Concat+Greater+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 27
baseline_size_bytes: 2435
```
