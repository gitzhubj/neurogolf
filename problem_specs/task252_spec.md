...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task252.json`。
- 架构方案已由 baseline ONNX 验证通过（27 节点，4879 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task252.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 27 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Concat+Mul+Pad+ReduceSum+Slice+Sub+Sum (27 nodes, 19 initializers, 4879 bytes)
