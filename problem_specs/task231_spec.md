...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task231.json`。
- 架构方案已由 baseline ONNX 验证通过（42 节点，3468 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task231.json`。
...
- `fusion_hint`: Study baseline: 42 nodes, ops=Abs+Add+And+Cast+Conv+Gather+Less+Mul+Pad+ReduceMax+ReduceSum+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Abs+Add+And+Cast+Conv+Gather+Less+Mul+Pad+ReduceMax+ReduceSum+Sub+Sum+Where (42 nodes, 13 initializers, 3468 bytes)
...
baseline_ops: Abs+Add+And+Cast+Conv+Gather+Less+Mul+Pad+ReduceMax+ReduceSum+Sub+Sum+Where
baseline_nodes: 42
baseline_size_bytes: 3468
```
