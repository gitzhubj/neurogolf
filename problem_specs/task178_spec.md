...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task178.json`。
- 架构方案已由 baseline ONNX 验证通过（123 节点，11303 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task178.json`。
...
- `fusion_hint`: Study baseline: 123 nodes, ops=Cast+Conv+Gather+Greater+Less+MatMul+Mul+Pad+ReduceSum+Reshape+Slice+Sub+Sum+Whe
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Conv+Gather+Greater+Less+MatMul+Mul+Pad+ReduceSum+Reshape+Slice+Sub+Sum+Where (123 nodes, 30 initializers, 11303 bytes)
...
baseline_ops: Cast+Conv+Gather+Greater+Less+MatMul+Mul+Pad+ReduceSum+Reshape+Slice+Sub+Sum+Where
baseline_nodes: 123
baseline_size_bytes: 11303
```
