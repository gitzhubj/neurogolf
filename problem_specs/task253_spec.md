...
- 输入尺寸: 13x13 (all same)，输出尺寸: 4x4 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task253.json`。
- 架构方案已由 baseline ONNX 验证通过（17 节点，3510 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task253.json`。
...
- `fusion_hint`: Study baseline: 17 nodes, ops=Cast+Conv+Gather+Greater+Pad+ReduceMax+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Conv+Gather+Greater+Pad+ReduceMax+Sum+Where (17 nodes, 13 initializers, 3510 bytes)
...
baseline_ops: Cast+Conv+Gather+Greater+Pad+ReduceMax+Sum+Where
baseline_nodes: 17
baseline_size_bytes: 3510
```
