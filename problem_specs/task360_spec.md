...
- 输入尺寸: 10x9 (all same)，输出尺寸: 10x4 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 5, 6, 7, 8]，输出颜色: [0, 1, 2, 3, 4, 6, 7, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task360.json`。
- 架构方案已由 baseline ONNX 验证通过（10 节点，1399 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task360.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Mul+Pad+ReduceMax+Slice+Sub+Sum (10 nodes, 15 initializers, 1399 bytes)
