...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task244.json`。
- 架构方案已由 baseline ONNX 验证通过（66 节点，6538 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task244.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — And+Cast+Equal+Gather+Pad+ReduceMax+ReduceSum+Sum+Where (66 nodes, 31 initializers, 6538 bytes)
...
baseline_ops: And+Cast+Equal+Gather+Pad+ReduceMax+ReduceSum+Sum+Where
baseline_nodes: 66
baseline_size_bytes: 6538
```
