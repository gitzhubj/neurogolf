...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task369.json`。
- 架构方案已由 baseline ONNX 验证通过（24 节点，4692 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task369.json`。
...
- `fusion_hint`: Study baseline: 24 nodes, ops=Concat+Conv+Min+Mul+Pad+Relu+Slice+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Concat+Conv+Min+Mul+Pad+Relu+Slice+Sub+Sum (24 nodes, 11 initializers, 4692 bytes)
...
baseline_ops: Concat+Conv+Min+Mul+Pad+Relu+Slice+Sub+Sum
baseline_nodes: 24
baseline_size_bytes: 4692
```
