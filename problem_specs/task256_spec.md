...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task256.json`。
- 架构方案已由 baseline ONNX 验证通过（38 节点，8793 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task256.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 38 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Concat+Max+Min+Mul+Pad+ReduceMax+ReduceSum+Relu+Slice+Sub+Sum (38 nodes, 17 initializers, 8793 bytes)
...
baseline_ops: Concat+Max+Min+Mul+Pad+ReduceMax+ReduceSum+Relu+Slice+Sub+Sum
baseline_nodes: 38
baseline_size_bytes: 8793
```
