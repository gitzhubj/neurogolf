...
- 训练样本: 2 个，测试样本: 1 个，ARC-GEN 样本: 261 个。
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task345.json`。
- 架构方案已由 baseline ONNX 验证通过（30 节点，6721 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task345.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — ArgMax+Cast+Concat+Greater+Max+Min+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where (30 nodes, 13 initializers, 6721 bytes)
...
baseline_ops: ArgMax+Cast+Concat+Greater+Max+Min+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 30
baseline_size_bytes: 6721
```
