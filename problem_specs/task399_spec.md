...
- 训练样本: 8 个，测试样本: 3 个，ARC-GEN 样本: 262 个。
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task399.json`。
- 架构方案已由 baseline ONNX 验证通过（8 节点，1907 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task399.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Abs+ArgMax+Gather+Neg+Pad+ReduceSum+Sub (8 nodes, 4 initializers, 1907 bytes)
...
baseline_ops: Abs+ArgMax+Gather+Neg+Pad+ReduceSum+Sub
baseline_nodes: 8
baseline_size_bytes: 1907
```
