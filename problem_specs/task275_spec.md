...
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 8]，输出颜色: [0, 1, 2, 3, 4]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task275.json`。
- 架构方案已由 baseline ONNX 验证通过（253 节点，24193 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task275.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 253 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — And+Cast+Concat+Gather+Greater+Less+MatMul+Mul+Pad+ReduceSum+Reshape+Sub+Sum+Transpose+Where (253 nodes, 27 initializers, 24193 bytes)
...
baseline_ops: And+Cast+Concat+Gather+Greater+Less+MatMul+Mul+Pad+ReduceSum+Reshape+Sub+Sum+Transpose+Where
baseline_nodes: 253
baseline_size_bytes: 24193
```
