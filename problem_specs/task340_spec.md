...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task340.json`。
- 架构方案已由 baseline ONNX 验证通过（71 节点，6593 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task340.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 71 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — And+ArgMax+Cast+Equal+Greater+Mul+OneHot+Pad+ReduceMax+ReduceSum+Slice+Squeeze+Sub+Sum+Where (71 nodes, 26 initializers, 6593 bytes)
...
baseline_ops: And+ArgMax+Cast+Equal+Greater+Mul+OneHot+Pad+ReduceMax+ReduceSum+Slice+Squeeze+Sub+Sum+Where
baseline_nodes: 71
baseline_size_bytes: 6593
```
