...
- 输入尺寸: 5 unique sizes，输出尺寸: 3x3 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 5, 6, 7, 8]，输出颜色: [0, 4, 6, 7, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task263.json`。
- 架构方案已由 baseline ONNX 验证通过（140 节点，10867 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task263.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 140 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — And+Cast+Greater+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where (140 nodes, 27 initializers, 10867 bytes)
...
baseline_ops: And+Cast+Greater+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 140
baseline_size_bytes: 10867
```
