...
- 输入尺寸: 7x7 (all same)，输出尺寸: 7x7 (all same)。
- 同尺寸变换: 是。
- 输入颜色: [0, 2, 3, 4, 6, 8]，输出颜色: [0, 2, 4, 6]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task267.json`。
- 架构方案已由 baseline ONNX 验证通过（9 节点，1624 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task267.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Equal+Mul+Pad+ReduceSum+Slice+Sub (9 nodes, 13 initializers, 1624 bytes)
...
baseline_ops: Equal+Mul+Pad+ReduceSum+Slice+Sub
baseline_nodes: 9
baseline_size_bytes: 1624
```
