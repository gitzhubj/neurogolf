...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 3, 4, 5, 8]，输出颜色: [0, 1, 3, 4, 5, 8]。
- 颜色集一致: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task224.json`。
- 架构方案已由 baseline ONNX 验证通过（54 节点，6314 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task224.json`。
...
- `fusion_hint`: Study baseline: 54 nodes, ops=Cast+Conv+Greater+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Conv+Greater+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where (54 nodes, 17 initializers, 6314 bytes)
...
arcgen_samples: 262
same_size: True
colors_in: [0, 1, 3, 4, 5, 8]
colors_out: [0, 1, 3, 4, 5, 8]
locality: k
single_linear_conv_possible: no
recommended_architecture: conv_with_logic
recommended_kernel: 3x3
