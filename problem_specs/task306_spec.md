...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task306.json`。
- 架构方案已由 baseline ONNX 验证通过（33 节点，4101 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task306.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — And+Cast+Concat+Greater+Not+Or+Pad+ReduceSum+Slice+Tile (33 nodes, 32 initializers, 4101 bytes)
...
baseline_ops: And+Cast+Concat+Greater+Not+Or+Pad+ReduceSum+Slice+Tile
baseline_nodes: 33
baseline_size_bytes: 4101
```
