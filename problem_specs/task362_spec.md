...
- 同尺寸变换: 是。
- 输入颜色: [0, 2, 3, 4, 5, 6, 8]，输出颜色: [0, 2, 3, 4, 6, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task362.json`。
- 架构方案已由 baseline ONNX 验证通过（26 节点，2814 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task362.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — ArgMax+Cast+Concat+Equal+Gather+Greater+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where (26 nodes, 11 initializers, 2814 bytes)
...
arcgen_samples: 262
same_size: True
colors_in: [0, 2, 3, 4, 5, 6, 8]
colors_out: [0, 2, 3, 4, 6, 8]
locality: global
single_linear_conv_possible: no
recommended_architecture: reduce_with_where
recommended_kernel: not_needed
...
baseline_ops: ArgMax+Cast+Concat+Equal+Gather+Greater+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 26
baseline_size_bytes: 2814
```
