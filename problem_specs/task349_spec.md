...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task349.json`。
- 架构方案已由 baseline ONNX 验证通过（89 节点，8755 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task349.json`。
...
- `fusion_hint`: Study baseline: 89 nodes, ops=And+Cast+Concat+Conv+Gather+Greater+Less+MaxPool+Mul+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — And+Cast+Concat+Conv+Gather+Greater+Less+MaxPool+Mul+Sub+Sum+Where (89 nodes, 22 initializers, 8755 bytes)
...
baseline_ops: And+Cast+Concat+Conv+Gather+Greater+Less+MaxPool+Mul+Sub+Sum+Where
baseline_nodes: 89
baseline_size_bytes: 8755
```
