...
- 输入尺寸: 6x6 (all same)，输出尺寸: 6x6 (all same)。
- 同尺寸变换: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task225.json`。
- 架构方案已由 baseline ONNX 验证通过（31 节点，5386 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task225.json`。
...
- `fusion_hint`: Study baseline: 31 nodes, ops=Concat+Conv+Mul+Pad+ReduceSum+Slice+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Concat+Conv+Mul+Pad+ReduceSum+Slice+Sub+Sum (31 nodes, 17 initializers, 5386 bytes)
...
baseline_ops: Concat+Conv+Mul+Pad+ReduceSum+Slice+Sub+Sum
baseline_nodes: 31
baseline_size_bytes: 5386
```
