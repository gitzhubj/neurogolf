...
- 训练样本: 2 个，测试样本: 1 个，ARC-GEN 样本: 28 个。
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task336.json`。
- 架构方案已由 baseline ONNX 验证通过（54 节点，5276 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task336.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 54 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — And+Cast+Concat+CumSum+Equal+Gather+Greater+Not+Or+Pad+ReduceMax+Slice (54 nodes, 12 initializers, 5276 bytes)
...
baseline_ops: And+Cast+Concat+CumSum+Equal+Gather+Greater+Not+Or+Pad+ReduceMax+Slice
baseline_nodes: 54
baseline_size_bytes: 5276
```
