...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task383.json`。
- 架构方案已由 baseline ONNX 验证通过（57 节点，6805 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task383.json`。
...
- `fusion_hint`: Study baseline: 57 nodes, ops=And+Cast+Concat+Conv+Equal+Greater+Mul+Or+ReduceSum+Slice+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — And+Cast+Concat+Conv+Equal+Greater+Mul+Or+ReduceSum+Slice+Sub+Sum+Where (57 nodes, 11 initializers, 6805 bytes)
...
baseline_ops: And+Cast+Concat+Conv+Equal+Greater+Mul+Or+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 57
baseline_size_bytes: 6805
```
