...
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 5, 7, 8]，输出颜色: [0, 1, 2, 5, 7, 8]。
- 颜色集一致: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task295.json`。
- 架构方案已由 baseline ONNX 验证通过（22 节点，2771 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task295.json`。
...
- `fusion_hint`: Study baseline: 22 nodes, ops=Cast+Conv+Greater+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Conv+Greater+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where (22 nodes, 11 initializers, 2771 bytes)
...
arcgen_samples: 262
same_size: False
colors_in: [0, 1, 2, 5, 7, 8]
colors_out: [0, 1, 2, 5, 7, 8]
locality: k
single_linear_conv_possible: no
recommended_architecture: conv_with_logic
recommended_kernel: 3x3
...
baseline_ops: Cast+Conv+Greater+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 22
baseline_size_bytes: 2771
```
