...
- 输入尺寸: 3x3 (all same)，输出尺寸: 4 unique sizes。
- 同尺寸变换: 否（输入/输出尺寸不同）。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task269.json`。
- 架构方案已由 baseline ONNX 验证通过（29 节点，3349 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task269.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 29 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — And+Div+Equal+Floor+Less+Mul+ReduceSum+Slice+Where (29 nodes, 18 initializers, 3349 bytes)
...
baseline_ops: And+Div+Equal+Floor+Less+Mul+ReduceSum+Slice+Where
baseline_nodes: 29
baseline_size_bytes: 3349
```
