...
- 核心变换：红(2)阶梯形每列从底部统计连续2的高度h，底部floor(h/2)个格由红变浅蓝(8)。
...
- `fusion_hint`: Gather + supporting ops. 19 nodes total.
- `approach`: Use Gather-based lookup or spatial permutation — no Conv needed.

Baseline 实际架构: **Gather-based multi-op** — And+Cast+Concat+CumSum+Gather+Greater+GreaterOrEqual+Not+Or+Pad+Slice+Sub (19 nodes, 11 initializers, 2183 bytes)
...
baseline_ops: And+Cast+Concat+CumSum+Gather+Greater+GreaterOrEqual+Not+Or+Pad+Slice+Sub
baseline_nodes: 19
baseline_size_bytes: 2183
```
