...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task184.json`。
- 架构方案已由 baseline ONNX 验证通过（51 节点，6934 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task184.json`。
...
- `fusion_hint`: Study baseline: 51 nodes, ops=And+Cast+Concat+Conv+CumSum+Equal+Greater+MatMul+Mul+Pad+ReduceMax+ReduceSum+Res
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — And+Cast+Concat+Conv+CumSum+Equal+Greater+MatMul+Mul+Pad+ReduceMax+ReduceSum+Reshape+Sub+Transpose+Where (51 nodes, 15 initializers, 6934 bytes)
...
baseline_ops: And+Cast+Concat+Conv+CumSum+Equal+Greater+MatMul+Mul+Pad+ReduceMax+ReduceSum+Reshape+Sub+Transpose+Where
baseline_nodes: 51
baseline_size_bytes: 6934
```
