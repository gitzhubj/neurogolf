...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 2, 4, 6, 7, 8, 9]，输出颜色: [0, 1, 2, 3, 4, 6, 7, 8, 9]。
- 颜色集一致: 否。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task397.json`。
- 架构方案已由 baseline ONNX 验证通过（77 节点，7101 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task397.json`。
...
- `fusion_hint`: Study baseline: 77 nodes, ops=Add+And+Cast+Concat+Conv+Equal+Gather+Greater+Pad+Slice+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Add+And+Cast+Concat+Conv+Equal+Gather+Greater+Pad+Slice+Sub+Sum (77 nodes, 30 initializers, 7101 bytes)
...
baseline_ops: Add+And+Cast+Concat+Conv+Equal+Gather+Greater+Pad+Slice+Sub+Sum
baseline_nodes: 77
baseline_size_bytes: 7101
```
