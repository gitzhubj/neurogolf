...
- 输入尺寸: 4 unique sizes，输出尺寸: 1x1 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 5, 6, 7, 8]，输出颜色: [2, 5, 6, 7]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task291.json`。
- 架构方案已由 baseline ONNX 验证通过（22 节点，6009 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task291.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — Add+ArgMax+Cast+Equal+Greater+Mul+ReduceMax+ReduceSum+Reshape+Sub+Sum+Where (22 nodes, 9 initializers, 6009 bytes)
...
baseline_ops: Add+ArgMax+Cast+Equal+Greater+Mul+ReduceMax+ReduceSum+Reshape+Sub+Sum+Where
baseline_nodes: 22
baseline_size_bytes: 6009
```
