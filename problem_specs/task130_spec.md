...
- `fusion_hint`: Study baseline: 10 nodes, ops=Conv+Gather+MaxPool+Mul+Pad+ReduceMax+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Conv+Gather+MaxPool+Mul+Pad+ReduceMax+Sub+Sum (10 nodes, 5 initializers, 1293 bytes)
...
baseline_ops: Conv+Gather+MaxPool+Mul+Pad+ReduceMax+Sub+Sum
baseline_nodes: 10
baseline_size_bytes: 1293
```
