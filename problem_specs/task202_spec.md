...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 2, 3, 4, 5, 8]，输出颜色: [0, 1, 2, 3, 4, 5, 8]。
- 颜色集一致: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task202.json`。
- 架构方案已由 baseline ONNX 验证通过（31 节点，3582 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task202.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — And+Cast+Greater+Less+Mul+Not+Or+ReduceMax+ReduceSum+Slice+Where (31 nodes, 5 initializers, 3582 bytes)
...
baseline_ops: And+Cast+Greater+Less+Mul+Not+Or+ReduceMax+ReduceSum+Slice+Where
baseline_nodes: 31
baseline_size_bytes: 3582
```
