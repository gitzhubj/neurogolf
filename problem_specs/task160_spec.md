...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task160.json`。
- 架构方案已由 baseline ONNX 验证通过（18 节点，2362 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task160.json`。
...
- `fusion_hint`: Study baseline: 18 nodes, ops=Cast+Concat+Conv+Greater+Pad+ReduceSum+Slice+Sub+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Concat+Conv+Greater+Pad+ReduceSum+Slice+Sub+Where (18 nodes, 13 initializers, 2362 bytes)
...
baseline_ops: Cast+Concat+Conv+Greater+Pad+ReduceSum+Slice+Sub+Where
baseline_nodes: 18
baseline_size_bytes: 2362
```
