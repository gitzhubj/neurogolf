...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 2, 3, 4, 5, 6, 7, 8]，输出颜色: [0, 2, 3, 6, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task161.json`。
- 架构方案已由 baseline ONNX 验证通过（51 节点，6008 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task161.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — And+Cast+Concat+CumSum+Equal+Greater+Not+Or+Pad+ReduceSum+Slice (51 nodes, 18 initializers, 6008 bytes)
...
baseline_ops: And+Cast+Concat+CumSum+Equal+Greater+Not+Or+Pad+ReduceSum+Slice
baseline_nodes: 51
baseline_size_bytes: 6008
```
