...
- 输入尺寸: 4x14 (all same)，输出尺寸: 4x4 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 4, 9]，输出颜色: [0, 1, 4, 9]。
...
- 训练样本: 5 个，测试样本: 1 个，ARC-GEN 样本: 261 个。
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task321.json`。
- 架构方案已由 baseline ONNX 验证通过（19 节点，2449 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task321.json`。
...
- `fusion_hint`: Study baseline: 19 nodes, ops=Conv+Mul+Pad+Slice+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Conv+Mul+Pad+Slice+Sub+Sum (19 nodes, 11 initializers, 2449 bytes)
...
baseline_ops: Conv+Mul+Pad+Slice+Sub+Sum
baseline_nodes: 19
baseline_size_bytes: 2449
```
