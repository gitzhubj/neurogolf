...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task137.json`。
- 架构方案已由 baseline ONNX 验证通过（36 节点，3864 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task137.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 36 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Abs+And+Cast+Concat+Div+Equal+Max+Mul+Not+Or+ReduceSum+Slice+Sub (36 nodes, 9 initializers, 3864 bytes)
...
baseline_ops: Abs+And+Cast+Concat+Div+Equal+Max+Mul+Not+Or+ReduceSum+Slice+Sub
baseline_nodes: 36
baseline_size_bytes: 3864
```
