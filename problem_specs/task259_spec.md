...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task259.json`。
- 架构方案已由 baseline ONNX 验证通过（30 节点，3882 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task259.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 30 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Add+And+ArgMax+Cast+Concat+Gather+Greater+LessOrEqual+Pad+ReduceSum+Slice+Sub+Unsqueeze+Where (30 nodes, 15 initializers, 3882 bytes)
...
baseline_ops: Add+And+ArgMax+Cast+Concat+Gather+Greater+LessOrEqual+Pad+ReduceSum+Slice+Sub+Unsqueeze+Where
baseline_nodes: 30
baseline_size_bytes: 3882
```
