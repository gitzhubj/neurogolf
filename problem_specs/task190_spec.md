...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task190.json`。
- 架构方案已由 baseline ONNX 验证通过（63 节点，8781 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task190.json`。
...
- `fusion_hint`: Study baseline: 63 nodes, ops=And+Conv+Equal+Greater+Mul+Or+Pad+ReduceMax+Slice+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — And+Conv+Equal+Greater+Mul+Or+Pad+ReduceMax+Slice+Where (63 nodes, 40 initializers, 8781 bytes)
...
baseline_ops: And+Conv+Equal+Greater+Mul+Or+Pad+ReduceMax+Slice+Where
baseline_nodes: 63
baseline_size_bytes: 8781
```
