...
- 输入尺寸: 24x24 (all same)，输出尺寸: 5x5 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 5, 6, 8]，输出颜色: [0, 3, 5, 6, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task400.json`。
- 架构方案已由 baseline ONNX 验证通过（25 节点，3021 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task400.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — Add+ArgMax+Cast+Gather+Greater+Pad+ReduceMax+ReduceSum+Reshape+Slice+Where (25 nodes, 6 initializers, 3021 bytes)
