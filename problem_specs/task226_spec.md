...
- 训练样本: 3 个，测试样本: 1 个，ARC-GEN 样本: 129 个。
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task226.json`。
- 架构方案已由 baseline ONNX 验证通过（37 节点，4694 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task226.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — Cast+Concat+CumSum+Equal+Greater+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where (37 nodes, 15 initializers, 4694 bytes)
...
baseline_ops: Cast+Concat+CumSum+Equal+Greater+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 37
baseline_size_bytes: 4694
```
