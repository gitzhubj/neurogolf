...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task374.json`。
- 架构方案已由 baseline ONNX 验证通过（144 节点，16450 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task374.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 144 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Cast+Concat+Max+Min+Mul+Pad+ReduceMax+Relu+Slice+Sub+Sum (144 nodes, 27 initializers, 16450 bytes)
...
baseline_ops: Cast+Concat+Max+Min+Mul+Pad+ReduceMax+Relu+Slice+Sub+Sum
baseline_nodes: 144
baseline_size_bytes: 16450
```
