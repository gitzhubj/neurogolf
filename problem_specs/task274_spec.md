...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task274.json`。
- 架构方案已由 baseline ONNX 验证通过（16 节点，3619 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task274.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — ArgMax+Cast+Gather+Greater+Pad+ReduceSum+Slice+Squeeze+Sub (16 nodes, 11 initializers, 3619 bytes)
...
baseline_ops: ArgMax+Cast+Gather+Greater+Pad+ReduceSum+Slice+Squeeze+Sub
baseline_nodes: 16
baseline_size_bytes: 3619
```
