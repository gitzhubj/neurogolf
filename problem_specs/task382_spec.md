...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task382.json`。
- 架构方案已由 baseline ONNX 验证通过（322 节点，65997 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task382.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 322 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Concat+Gather+Greater+Less+MatMul+Mul+ReduceMax+ReduceSum+Reshape+Sub+Sum+Where (322 nodes, 27 initializers, 65997 bytes)
...
baseline_ops: Concat+Gather+Greater+Less+MatMul+Mul+ReduceMax+ReduceSum+Reshape+Sub+Sum+Where
baseline_nodes: 322
baseline_size_bytes: 65997
```
