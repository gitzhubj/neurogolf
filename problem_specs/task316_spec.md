...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task316.json`。
- 架构方案已由 baseline ONNX 验证通过（49 节点，5537 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task316.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 49 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Concat+CumSum+Equal+Mul+Pad+ReduceMax+ReduceSum+Sub+Sum+Where (49 nodes, 15 initializers, 5537 bytes)
...
baseline_ops: Concat+CumSum+Equal+Mul+Pad+ReduceMax+ReduceSum+Sub+Sum+Where
baseline_nodes: 49
baseline_size_bytes: 5537
```
