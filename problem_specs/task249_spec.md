...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task249.json`。
- 架构方案已由 baseline ONNX 验证通过（17 节点，2901 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task249.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 17 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Concat+Greater+Pad+ReduceMax+ReduceSum+Slice+Where (17 nodes, 14 initializers, 2901 bytes)
...
baseline_ops: Concat+Greater+Pad+ReduceMax+ReduceSum+Slice+Where
baseline_nodes: 17
baseline_size_bytes: 2901
```
