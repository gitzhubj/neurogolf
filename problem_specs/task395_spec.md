...
- 输入尺寸: 6x3 (all same)，输出尺寸: 3x3 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 9]，输出颜色: [0, 2]。
- 颜色集一致: 否。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task395.json`。
- 架构方案已由 baseline ONNX 验证通过（12 节点，1479 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task395.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Mul+Pad+ReduceMax+Slice+Sub+Sum (12 nodes, 10 initializers, 1479 bytes)
