...
- 训练样本: 2 个，测试样本: 1 个，ARC-GEN 样本: 18 个。
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task299.json`。
- 架构方案已由 baseline ONNX 验证通过（19 节点，1980 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task299.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 19 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — And+Cast+Concat+Not+Or+Pad+ReduceMax+Slice (19 nodes, 8 initializers, 1980 bytes)
...
baseline_ops: And+Cast+Concat+Not+Or+Pad+ReduceMax+Slice
baseline_nodes: 19
baseline_size_bytes: 1980
```
