...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task196.json`。
- 架构方案已由 baseline ONNX 验证通过（30 节点，3457 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task196.json`。
...
- `fusion_hint`: Study baseline: 30 nodes, ops=Conv+Greater+Less+Mul+Or+Pad+Slice+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Conv+Greater+Less+Mul+Or+Pad+Slice+Sub+Sum+Where (30 nodes, 14 initializers, 3457 bytes)
...
baseline_ops: Conv+Greater+Less+Mul+Or+Pad+Slice+Sub+Sum+Where
baseline_nodes: 30
baseline_size_bytes: 3457
```
