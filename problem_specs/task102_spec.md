...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task102.json`。
- 架构方案已由 baseline ONNX 验证通过（149 节点，22167 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task102.json`。
...
- `fusion_hint`: Study baseline: 149 nodes, ops=Cast+Concat+Conv+Greater+Mul+Or+Pad+Slice+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Concat+Conv+Greater+Mul+Or+Pad+Slice+Sub+Sum (149 nodes, 12 initializers, 22167 bytes)
...
baseline_ops: Cast+Concat+Conv+Greater+Mul+Or+Pad+Slice+Sub+Sum
baseline_nodes: 149
baseline_size_bytes: 22167
```
