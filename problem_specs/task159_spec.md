...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task159.json`。
- 架构方案已由 baseline ONNX 验证通过（57 节点，12966 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task159.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 57 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Add+ArgMax+Cast+Gather+Less+Mul+Pad+ReduceMax+ReduceSum+Sub+Sum+Where (57 nodes, 16 initializers, 12966 bytes)
...
baseline_ops: Add+ArgMax+Cast+Gather+Less+Mul+Pad+ReduceMax+ReduceSum+Sub+Sum+Where
baseline_nodes: 57
baseline_size_bytes: 12966
```
