...
- 输入尺寸: 9x3 (all same)，输出尺寸: 3x3 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [1, 2, 3, 4, 5, 6, 7, 8, 9]，输出颜色: [1, 3, 4, 5, 6, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task146.json`。
- 架构方案已由 baseline ONNX 验证通过（29 节点，1931 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task146.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — ArgMax+Cast+Equal+Less+Max+Pad+ReduceSum+Slice+Transpose+Where (29 nodes, 12 initializers, 1931 bytes)
...
baseline_ops: ArgMax+Cast+Equal+Less+Max+Pad+ReduceSum+Slice+Transpose+Where
baseline_nodes: 29
baseline_size_bytes: 1931
```
