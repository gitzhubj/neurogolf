...
- `fusion_hint`: Study baseline: 13 nodes, ops=And+Cast+Conv+Gather+Greater+Less+Pad+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — And+Cast+Conv+Gather+Greater+Less+Pad+Sub+Sum (13 nodes, 7 initializers, 1404 bytes)
...
baseline_ops: And+Cast+Conv+Gather+Greater+Less+Pad+Sub+Sum
baseline_nodes: 13
baseline_size_bytes: 1404
```
