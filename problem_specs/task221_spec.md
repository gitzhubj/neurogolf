...
- 输入尺寸: 3x3 (all same)，输出尺寸: 5 unique sizes。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 2, 3, 4, 6, 8]，输出颜色: [0, 2, 3, 4, 6, 8]。
- 颜色集一致: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task221.json`。
- 架构方案已由 baseline ONNX 验证通过（28 节点，13333 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task221.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 28 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Cast+Concat+ConvTranspose+Equal+ReduceSum+Slice+Sub+Sum+Where (28 nodes, 26 initializers, 13333 bytes)
...
baseline_ops: Cast+Concat+ConvTranspose+Equal+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 28
baseline_size_bytes: 13333
```
