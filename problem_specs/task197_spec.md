...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 2, 3, 4, 5, 6, 8]，输出颜色: [0, 1, 2, 3, 4, 5, 6, 8]。
- 颜色集一致: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task197.json`。
- 架构方案已由 baseline ONNX 验证通过（17 节点，1758 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task197.json`。
...
- `fusion_hint`: Study baseline: 17 nodes, ops=Abs+Conv+Less+MatMul+MaxPool+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Transpose+Whe
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Abs+Conv+Less+MatMul+MaxPool+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Transpose+Where (17 nodes, 7 initializers, 1758 bytes)
...
baseline_ops: Abs+Conv+Less+MatMul+MaxPool+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Transpose+Where
baseline_nodes: 17
baseline_size_bytes: 1758
```
