...
- 输入尺寸: 16x16 (all same)，输出尺寸: 16x16 (all same)。
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 2, 3, 4, 5, 6, 7, 8]，输出颜色: [1, 2, 3, 4, 5, 6, 7, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task305.json`。
- 架构方案已由 baseline ONNX 验证通过（6 节点，1865 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task305.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Equal+Mod+Mul+Pad+ReduceMax (6 nodes, 4 initializers, 1865 bytes)
...
baseline_ops: Equal+Mod+Mul+Pad+ReduceMax
baseline_nodes: 6
baseline_size_bytes: 1865
```
