...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task228.json`。
- 架构方案已由 baseline ONNX 验证通过（59 节点，4406 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task228.json`。
...
- `fusion_hint`: Study baseline: 59 nodes, ops=Abs+Cast+Clip+Concat+Conv+Equal+Gather+Max+Mul+Pad+ReduceSum+Slice+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Abs+Cast+Clip+Concat+Conv+Equal+Gather+Max+Mul+Pad+ReduceSum+Slice+Sub+Sum (59 nodes, 15 initializers, 4406 bytes)
...
baseline_ops: Abs+Cast+Clip+Concat+Conv+Equal+Gather+Max+Mul+Pad+ReduceSum+Slice+Sub+Sum
baseline_nodes: 59
baseline_size_bytes: 4406
```
