...
- 核心变换：众数填充：统计输入各颜色出现次数，用众数颜色填充整个输出。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 7 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — ArgMax+Expand+OneHot+Pad+ReduceSum+Slice+Unsqueeze (7 nodes, 10 initializers, 1196 bytes)
...
baseline_ops: ArgMax+Expand+OneHot+Pad+ReduceSum+Slice+Unsqueeze
baseline_nodes: 7
baseline_size_bytes: 1196
```
