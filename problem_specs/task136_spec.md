...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task136.json`。
- 架构方案已由 baseline ONNX 验证通过（31 节点，4980 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task136.json`。
...
- `fusion_hint`: Study baseline: 31 nodes, ops=Add+Cast+Concat+Conv+Gather+Greater+Mul+Pad+Slice+Sub
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Add+Cast+Concat+Conv+Gather+Greater+Mul+Pad+Slice+Sub (31 nodes, 17 initializers, 4980 bytes)
...
baseline_ops: Add+Cast+Concat+Conv+Gather+Greater+Mul+Pad+Slice+Sub
baseline_nodes: 31
baseline_size_bytes: 4980
```
