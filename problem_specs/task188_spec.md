...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task188.json`。
- 架构方案已由 baseline ONNX 验证通过（41 节点，4356 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task188.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 41 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Abs+Cast+Div+Greater+Less+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where (41 nodes, 15 initializers, 4356 bytes)
...
baseline_ops: Abs+Cast+Div+Greater+Less+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 41
baseline_size_bytes: 4356
```
