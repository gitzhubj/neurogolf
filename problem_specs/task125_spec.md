...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task125.json`。
- 架构方案已由 baseline ONNX 验证通过（28 节点，3032 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task125.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — And+Concat+CumSum+Greater+MaxPool+Not+Pad+ReduceSum+Slice+Sub (28 nodes, 10 initializers, 3032 bytes)
...
baseline_ops: And+Concat+CumSum+Greater+MaxPool+Not+Pad+ReduceSum+Slice+Sub
baseline_nodes: 28
baseline_size_bytes: 3032
```
