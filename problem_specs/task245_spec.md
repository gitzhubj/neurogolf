...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task245.json`。
- 架构方案已由 baseline ONNX 验证通过（47 节点，6141 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task245.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 47 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Add+And+ArgMax+Cast+Concat+Gather+Greater+Less+Mul+Not+ReduceMax+Reshape+Slice+Sub+Where (47 nodes, 16 initializers, 6141 bytes)
...
baseline_ops: Add+And+ArgMax+Cast+Concat+Gather+Greater+Less+Mul+Not+ReduceMax+Reshape+Slice+Sub+Where
baseline_nodes: 47
baseline_size_bytes: 6141
```
