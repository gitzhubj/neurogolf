...
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 6, 8, 9]，输出颜色: [0, 1, 2, 3, 4, 6, 8, 9]。
- 颜色集一致: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task327.json`。
- 架构方案已由 baseline ONNX 验证通过（17 节点，2235 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task327.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Mul+Pad+ReduceMax+Slice+Sum (17 nodes, 9 initializers, 2235 bytes)
...
arcgen_samples: 261
same_size: False
colors_in: [0, 1, 2, 3, 4, 6, 8, 9]
colors_out: [0, 1, 2, 3, 4, 6, 8, 9]
locality: global
single_linear_conv_possible: no
recommended_architecture: reduce_only
recommended_kernel: not_needed
...
baseline_ops: Mul+Pad+ReduceMax+Slice+Sum
baseline_nodes: 17
baseline_size_bytes: 2235
```
