...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task145.json`。
- 架构方案已由 baseline ONNX 验证通过（325 节点，31434 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task145.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 325 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — And+Cast+Concat+Equal+Gather+Mul+ReduceMax+ReduceMin+Sub+Sum+Where (325 nodes, 34 initializers, 31434 bytes)
...
baseline_ops: And+Cast+Concat+Equal+Gather+Mul+ReduceMax+ReduceMin+Sub+Sum+Where
baseline_nodes: 325
baseline_size_bytes: 31434
```
