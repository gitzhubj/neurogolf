...
- 输入尺寸: 3x3 (all same)，输出尺寸: 6x6 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task152.json`。
- 架构方案已由 baseline ONNX 验证通过（1 节点，7454 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task152.json`。
...
- `memory_priority`: Complex multi-op architecture (1 nodes). Study baseline directly.
- `fusion_hint`: Ops: GridSample...
- `approach`: Study baseline ONNX directly for optimal architecture.

Baseline 实际架构: **Custom multi-op** — GridSample (1 nodes, 1 initializers, 7454 bytes)
...
baseline_ops: GridSample
baseline_nodes: 1
baseline_size_bytes: 7454
```
