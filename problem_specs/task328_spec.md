...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task328.json`。
- 架构方案已由 baseline ONNX 验证通过（121 节点，16269 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task328.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 121 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — And+Cast+Equal+Gather+Greater+Less+Mul+OneHot+Or+Pad+ReduceMax+ReduceSum+Slice+Squeeze+Sub+Sum+Unsqueeze+Where (121 nodes, 27 initializers, 16269 bytes)
...
baseline_ops: And+Cast+Equal+Gather+Greater+Less+Mul+OneHot+Or+Pad+ReduceMax+ReduceSum+Slice+Squeeze+Sub+Sum+Unsqueeze+Where
baseline_nodes: 121
baseline_size_bytes: 16269
```
