...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 2, 3, 4]，输出颜色: [0, 1, 2, 3, 4, 5]。
- 颜色集一致: 否。
...
- 训练样本: 3 个，测试样本: 1 个，ARC-GEN 样本: 80 个。
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task200.json`。
- 架构方案已由 baseline ONNX 验证通过（91 节点，797905 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task200.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 91 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Add+Mul+ReduceSum+Sum (91 nodes, 41 initializers, 797905 bytes)
...
baseline_ops: Add+Mul+ReduceSum+Sum
baseline_nodes: 91
baseline_size_bytes: 797905
```
