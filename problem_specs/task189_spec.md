...
- 输入尺寸: 9x9 (all same)，输出尺寸: 6x6 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 5, 6, 7, 8]，输出颜色: [0, 1, 2, 4, 5, 6, 7]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task189.json`。
- 架构方案已由 baseline ONNX 验证通过（86 节点，8560 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task189.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 86 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Greater+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where (86 nodes, 55 initializers, 8560 bytes)
...
baseline_ops: Greater+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 86
baseline_size_bytes: 8560
```
