...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task279.json`。
- 架构方案已由 baseline ONNX 验证通过（41 节点，4658 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task279.json`。
...
- `fusion_hint`: Study baseline: 41 nodes, ops=Concat+Conv+Max+MaxPool+Mul+Pad+Slice+Sub+Sum
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Concat+Conv+Max+MaxPool+Mul+Pad+Slice+Sub+Sum (41 nodes, 8 initializers, 4658 bytes)
...
baseline_ops: Concat+Conv+Max+MaxPool+Mul+Pad+Slice+Sub+Sum
baseline_nodes: 41
baseline_size_bytes: 4658
```
