...
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 4, 7, 8]，输出颜色: [1, 2, 4, 7, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task339.json`。
- 架构方案已由 baseline ONNX 验证通过（12 节点，2038 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task339.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Add+ArgMax+Cast+Gather+Mul+OneHot+Pad+ReduceSum+Slice+Sub+Unsqueeze (12 nodes, 11 initializers, 2038 bytes)
...
arcgen_samples: 261
same_size: False
colors_in: [0, 1, 2, 4, 7, 8]
colors_out: [1, 2, 4, 7, 8]
locality: global
single_linear_conv_possible: no
recommended_architecture: reduce_only
recommended_kernel: not_needed
...
baseline_ops: Add+ArgMax+Cast+Gather+Mul+OneHot+Pad+ReduceSum+Slice+Sub+Unsqueeze
baseline_nodes: 12
baseline_size_bytes: 2038
```
