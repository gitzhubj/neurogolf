...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 2, 3, 5, 6]，输出颜色: [0, 1, 2, 3, 6]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task206.json`。
- 架构方案已由 baseline ONNX 验证通过（224 节点，20303 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task206.json`。
...
- `fusion_hint`: Study baseline: 224 nodes, ops=Cast+Concat+Conv+Gather+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Concat+Conv+Gather+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where (224 nodes, 33 initializers, 20303 bytes)
...
baseline_ops: Cast+Concat+Conv+Gather+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 224
baseline_size_bytes: 20303
```
