...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task168.json`。
- 架构方案已由 baseline ONNX 验证通过（44 节点，7297 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task168.json`。
...
- `fusion_hint`: Study baseline: 44 nodes, ops=Cast+Concat+Conv+Greater+Mul+Or+Pad+ReduceMax+Slice+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Concat+Conv+Greater+Mul+Or+Pad+ReduceMax+Slice+Sub+Sum+Where (44 nodes, 11 initializers, 7297 bytes)
...
baseline_ops: Cast+Concat+Conv+Greater+Mul+Or+Pad+ReduceMax+Slice+Sub+Sum+Where
baseline_nodes: 44
baseline_size_bytes: 7297
```
