...
- 训练样本: 2 个，测试样本: 1 个，ARC-GEN 样本: 4 个。
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task104.json`。
- 架构方案已由 baseline ONNX 验证通过（20 节点，6350 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task104.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Concat+Mul+Pad+ReduceMax+Slice+Sub+Sum (20 nodes, 16 initializers, 6350 bytes)
...
baseline_ops: Concat+Mul+Pad+ReduceMax+Slice+Sub+Sum
baseline_nodes: 20
baseline_size_bytes: 6350
```
