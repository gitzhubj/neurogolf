...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task112.json`。
- 架构方案已由 baseline ONNX 验证通过（44 节点，3987 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task112.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — Cast+Concat+Gather+Less+Max+Min+Mul+ReduceMax+ReduceSum+Reshape+Slice+Sub+Sum+Where (44 nodes, 18 initializers, 3987 bytes)
...
baseline_ops: Cast+Concat+Gather+Less+Max+Min+Mul+ReduceMax+ReduceSum+Reshape+Slice+Sub+Sum+Where
baseline_nodes: 44
baseline_size_bytes: 3987
```
