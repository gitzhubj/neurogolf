...
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 5, 6, 7, 8]，输出颜色: [1, 2, 3, 4, 6, 7, 8]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task213.json`。
- 架构方案已由 baseline ONNX 验证通过（385 节点，25091 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task213.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 385 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Cast+Equal+Greater+Less+Mul+Pad+ReduceMax+ReduceSum+Slice+Split+Sum+Unsqueeze+Where (385 nodes, 18 initializers, 25091 bytes)
...
baseline_ops: Cast+Equal+Greater+Less+Mul+Pad+ReduceMax+ReduceSum+Slice+Split+Sum+Unsqueeze+Where
baseline_nodes: 385
baseline_size_bytes: 25091
```
