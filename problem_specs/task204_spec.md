...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task204.json`。
- 架构方案已由 baseline ONNX 验证通过（71 节点，5801 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task204.json`。
...
- `fusion_hint`: Study baseline: 71 nodes, ops=Cast+Concat+Conv+Div+GatherND+Max+MaxPool+Min+Mul+Sub+Sum+Unsqueeze
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Concat+Conv+Div+GatherND+Max+MaxPool+Min+Mul+Sub+Sum+Unsqueeze (71 nodes, 13 initializers, 5801 bytes)
...
baseline_ops: Cast+Concat+Conv+Div+GatherND+Max+MaxPool+Min+Mul+Sub+Sum+Unsqueeze
baseline_nodes: 71
baseline_size_bytes: 5801
```
