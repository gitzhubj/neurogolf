...
- 同尺寸变换: 是。
- 输入颜色: [0, 3, 5, 6, 8]，输出颜色: [0, 3, 4, 5, 6, 8]。
- 颜色集一致: 否。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task126.json`。
- 架构方案已由 baseline ONNX 验证通过（29 节点，2765 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task126.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — Cast+Greater+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Where (29 nodes, 11 initializers, 2765 bytes)
...
baseline_ops: Cast+Greater+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Where
baseline_nodes: 29
baseline_size_bytes: 2765
```
