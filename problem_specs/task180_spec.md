...
- `fusion_hint`: Study baseline: 5 nodes, ops=Conv+Mul+Pad+ReduceMax+Slice
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Conv+Mul+Pad+ReduceMax+Slice (5 nodes, 5 initializers, 4414 bytes)
...
baseline_ops: Conv+Mul+Pad+ReduceMax+Slice
baseline_nodes: 5
baseline_size_bytes: 4414
```
