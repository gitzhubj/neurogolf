...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task348.json`。
- 架构方案已由 baseline ONNX 验证通过（29 节点，3000 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task348.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 29 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Abs+And+Concat+Div+Floor+Greater+Less+Mul+Not+Or+Pad+ReduceMax+ReduceSum+Slice+Sub (29 nodes, 12 initializers, 3000 bytes)
...
baseline_ops: Abs+And+Concat+Div+Floor+Greater+Less+Mul+Not+Or+Pad+ReduceMax+ReduceSum+Slice+Sub
baseline_nodes: 29
baseline_size_bytes: 3000
```
