...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task148.json`。
- 架构方案已由 baseline ONNX 验证通过（107 节点，10573 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task148.json`。
...
- `fusion_hint`: Study baseline: 107 nodes, ops=And+Cast+Concat+Conv+Greater+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — And+Cast+Concat+Conv+Greater+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where (107 nodes, 16 initializers, 10573 bytes)
...
baseline_ops: And+Cast+Concat+Conv+Greater+Less+Mul+Pad+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 107
baseline_size_bytes: 10573
```
