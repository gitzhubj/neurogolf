...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task381.json`。
- 架构方案已由 baseline ONNX 验证通过（20 节点，5721 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task381.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Concat+MaxPool+Mul+ReduceMax+Slice+Sub (20 nodes, 9 initializers, 5721 bytes)
...
baseline_ops: Concat+MaxPool+Mul+ReduceMax+Slice+Sub
baseline_nodes: 20
baseline_size_bytes: 5721
```
