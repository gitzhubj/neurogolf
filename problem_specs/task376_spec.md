...
- 训练样本: 2 个，测试样本: 1 个，ARC-GEN 样本: 36 个。
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task376.json`。
- 架构方案已由 baseline ONNX 验证通过（26 节点，2344 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task376.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — Add+Cast+Div+Gather+Less+Max+Min+Mod+Mul+ReduceMax+ReduceSum+Reshape+Sub+Where (26 nodes, 7 initializers, 2344 bytes)
...
baseline_ops: Add+Cast+Div+Gather+Less+Max+Min+Mod+Mul+ReduceMax+ReduceSum+Reshape+Sub+Where
baseline_nodes: 26
baseline_size_bytes: 2344
```
