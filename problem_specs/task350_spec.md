...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task350.json`。
- 架构方案已由 baseline ONNX 验证通过（19 节点，2670 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task350.json`。
...
- `memory_priority`: Complex multi-op architecture (19 nodes). Study baseline directly.
- `fusion_hint`: Ops: And+Cast+Concat+CumSum+Or+Pad+Slice+Xor...
- `approach`: Study baseline ONNX directly for optimal architecture.

Baseline 实际架构: **Custom multi-op** — And+Cast+Concat+CumSum+Or+Pad+Slice+Xor (19 nodes, 9 initializers, 2670 bytes)
...
baseline_ops: And+Cast+Concat+CumSum+Or+Pad+Slice+Xor
baseline_nodes: 19
baseline_size_bytes: 2670
```
