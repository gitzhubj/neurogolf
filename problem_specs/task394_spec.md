...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task394.json`。
- 架构方案已由 baseline ONNX 验证通过（78 节点，5312 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task394.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 78 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Add+ArgMax+Cast+Clip+Concat+Expand+Gather+Greater+Less+Mod+Mul+Pad+ReduceMax+ReduceSum+Reshape+Sign+Slice+Sub+Unsqueeze (78 nodes, 14 initializers, 5312 bytes)
...
baseline_ops: Add+ArgMax+Cast+Clip+Concat+Expand+Gather+Greater+Less+Mod+Mul+Pad+ReduceMax+ReduceSum+Reshape+Sign+Slice+Sub+Unsqueeze
baseline_nodes: 78
baseline_size_bytes: 5312
```
