...
- 核心变换：2x2同色过滤：仅保留属于至少一个2x2同色方块的颜色像素，去除孤立像素和细突起。
...
- `fusion_hint`: Study baseline: 14 nodes, ops=Cast+Conv+Greater+Mul+Pad+Slice+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Conv+Greater+Mul+Pad+Slice+Sum+Where (14 nodes, 12 initializers, 1925 bytes)
...
baseline_ops: Cast+Conv+Greater+Mul+Pad+Slice+Sum+Where
baseline_nodes: 14
baseline_size_bytes: 1925
```
