...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task358.json`。
- 架构方案已由 baseline ONNX 验证通过（132 节点，10944 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task358.json`。
...
- `fusion_hint`: Study baseline: 132 nodes, ops=Cast+Concat+Conv+Equal+Greater+Less+Mul+Or+Pad+ReduceSum+Slice+Sub+Sum+Where
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Cast+Concat+Conv+Equal+Greater+Less+Mul+Or+Pad+ReduceSum+Slice+Sub+Sum+Where (132 nodes, 37 initializers, 10944 bytes)
...
baseline_ops: Cast+Concat+Conv+Equal+Greater+Less+Mul+Or+Pad+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 132
baseline_size_bytes: 10944
```
