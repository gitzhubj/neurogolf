...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task251.json`。
- 架构方案已由 baseline ONNX 验证通过（53 节点，6906 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task251.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 53 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Cast+Concat+Max+MaxPool+Mul+Pad+ReduceSum+Relu+Slice+Sub (53 nodes, 13 initializers, 6906 bytes)
...
baseline_ops: Cast+Concat+Max+MaxPool+Mul+Pad+ReduceSum+Relu+Slice+Sub
baseline_nodes: 53
baseline_size_bytes: 6906
```
