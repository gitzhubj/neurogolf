...
- 输入尺寸: 12x12 (all same)，输出尺寸: 3x1 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task393.json`。
- 架构方案已由 baseline ONNX 验证通过（22 节点，2433 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task393.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 22 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Add+ArgMax+Concat+Mul+OneHot+Pad+ReduceSum+Slice+Sub+Unsqueeze (22 nodes, 12 initializers, 2433 bytes)
