...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 5, 8, 9]，输出颜色: [1, 3, 4, 5, 8, 9]。
- 颜色集一致: 否。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task198.json`。
- 架构方案已由 baseline ONNX 验证通过（51 节点，6979 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task198.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 51 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Cast+Concat+Greater+Max+MaxPool+Mul+ReduceMax+ReduceSum+Slice+Sub+Sum (51 nodes, 14 initializers, 6979 bytes)
...
baseline_ops: Cast+Concat+Greater+Max+MaxPool+Mul+ReduceMax+ReduceSum+Slice+Sub+Sum
baseline_nodes: 51
baseline_size_bytes: 6979
```
