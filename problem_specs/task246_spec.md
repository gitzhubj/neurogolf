...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task246.json`。
- 架构方案已由 baseline ONNX 验证通过（31 节点，3824 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task246.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — And+Cast+Concat+CumSum+Or+Pad+ReduceMax+Slice+Sum+Xor (31 nodes, 11 initializers, 3824 bytes)
...
baseline_ops: And+Cast+Concat+CumSum+Or+Pad+ReduceMax+Slice+Sum+Xor
baseline_nodes: 31
baseline_size_bytes: 3824
```
