...
- 输入尺寸: 4 unique sizes，输出尺寸: 3x1 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task391.json`。
- 架构方案已由 baseline ONNX 验证通过（26 节点，2841 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task391.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 26 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Add+ArgMax+Concat+Mul+OneHot+Pad+ReduceSum+Slice+Sub+Unsqueeze (26 nodes, 12 initializers, 2841 bytes)
...
baseline_ops: Add+ArgMax+Concat+Mul+OneHot+Pad+ReduceSum+Slice+Sub+Unsqueeze
baseline_nodes: 26
baseline_size_bytes: 2841
```
