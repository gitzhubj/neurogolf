...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task105.json`。
- 架构方案已由 baseline ONNX 验证通过（50 节点，35291 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task105.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — Concat+Greater+Max+MaxPool+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where (50 nodes, 12 initializers, 35291 bytes)
...
baseline_ops: Concat+Greater+Max+MaxPool+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum+Where
baseline_nodes: 50
baseline_size_bytes: 35291
```
