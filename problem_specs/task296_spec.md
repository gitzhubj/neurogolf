...
- 输入尺寸: 5x7 (all same)，输出尺寸: 3x3 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task296.json`。
- 架构方案已由 baseline ONNX 验证通过（10 节点，2843 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task296.json`。
...
Baseline 实际架构: **Reduce + arithmetic** — Cast+Concat+Greater+MatMul+Pad+ReduceSum+Reshape+Slice+Sub (10 nodes, 11 initializers, 2843 bytes)
...
baseline_ops: Cast+Concat+Greater+MatMul+Pad+ReduceSum+Reshape+Slice+Sub
baseline_nodes: 10
baseline_size_bytes: 2843
```
