...
- 输入尺寸: 9x9 (all same)，输出尺寸: 4 unique sizes。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 4]，输出颜色: [0, 4]。
- 颜色集一致: 是。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task384.json`。
- 架构方案已由 baseline ONNX 验证通过（39 节点，4542 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task384.json`。
...
Baseline 实际架构: **Reduce + Where conditional** — Add+ArgMax+Cast+Gather+Less+Mul+ReduceMax+Reshape+Slice+Sub+Where (39 nodes, 13 initializers, 4542 bytes)
