...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task370.json`。
- 架构方案已由 baseline ONNX 验证通过（316 节点，37248 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task370.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 316 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — And+Cast+Equal+Greater+Not+Or+Pad+ReduceMax+ReduceSum+Slice (316 nodes, 134 initializers, 37248 bytes)
...
baseline_ops: And+Cast+Equal+Greater+Not+Or+Pad+ReduceMax+ReduceSum+Slice
baseline_nodes: 316
baseline_size_bytes: 37248
```
