...
- 输入尺寸: 13x13 (all same)，输出尺寸: 3x3 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3, 4, 6, 7, 8]，输出颜色: [0, 2, 3, 4, 7]。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task121.json`。
- 架构方案已由 baseline ONNX 验证通过（24 节点，3339 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task121.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 24 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Add+ArgMax+Concat+Gather+Mul+OneHot+Pad+ReduceSum+Slice+Sub+Unsqueeze (24 nodes, 17 initializers, 3339 bytes)
...
baseline_ops: Add+ArgMax+Concat+Gather+Mul+OneHot+Pad+ReduceSum+Slice+Sub+Unsqueeze
baseline_nodes: 24
baseline_size_bytes: 3339
```
