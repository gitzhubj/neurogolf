...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 2, 3, 4, 6, 7, 9]，输出颜色: [0, 1, 2, 3, 4, 6, 7, 8, 9]。
- 颜色集一致: 否。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task341.json`。
- 架构方案已由 baseline ONNX 验证通过（99 节点，11979 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task341.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 99 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — Max+Min+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum (99 nodes, 52 initializers, 11979 bytes)
...
baseline_ops: Max+Min+Mul+Pad+ReduceMax+ReduceSum+Slice+Sub+Sum
baseline_nodes: 99
baseline_size_bytes: 11979
```
