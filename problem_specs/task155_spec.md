...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + Where. 12 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + Where conditional** — Cast+Gather+Greater+Less+ReduceSum+Squeeze+Sub+Where (12 nodes, 6 initializers, 1347 bytes)
...
baseline_ops: Cast+Gather+Greater+Less+ReduceSum+Squeeze+Sub+Where
baseline_nodes: 12
baseline_size_bytes: 1347
```
