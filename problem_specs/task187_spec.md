...
- 同尺寸变换: 是。
- 输入颜色: [0, 1, 4, 7, 8]，输出颜色: [1, 2, 3, 4, 7, 8]。
- 颜色集一致: 否。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task187.json`。
- 架构方案已由 baseline ONNX 验证通过（87 节点，8852 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task187.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 87 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — And+Cast+Concat+Gather+Greater+MaxPool+Not+Or+ReduceSum (87 nodes, 14 initializers, 8852 bytes)
...
baseline_ops: And+Cast+Concat+Gather+Greater+MaxPool+Not+Or+ReduceSum
baseline_nodes: 87
baseline_size_bytes: 8852
```
