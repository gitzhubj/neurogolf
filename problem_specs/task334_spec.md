...
- 输入尺寸: 5x5 (all same)，输出尺寸: 3x3 (all same)。
- 同尺寸变换: 否（输入/输出尺寸不同）。
- 输入颜色: [0, 1, 2, 3]，输出颜色: [0, 5]。
- 颜色集一致: 否。
...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task334.json`。
- 架构方案已由 baseline ONNX 验证通过（5 节点，2063 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task334.json`。
...
- `fusion_hint`: ReduceSum/ReduceMax + Greater/Equal + arithmetic. 5 nodes.
- `approach`: Use ReduceSum/ReduceMax + threshold logic — no Conv needed.

Baseline 实际架构: **Reduce + arithmetic** — ArgMax+Gather+Pad+ReduceSum+Slice (5 nodes, 8 initializers, 2063 bytes)
...
baseline_ops: ArgMax+Gather+Pad+ReduceSum+Slice
baseline_nodes: 5
baseline_size_bytes: 2063
```
