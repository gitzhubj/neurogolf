...
- 输入样例可能包含多种尺寸和颜色组合，详见 `input/task325.json`。
- 架构方案已由 baseline ONNX 验证通过（39 节点，5072 字节）。

> **建议**: 运行 `python tools/task_scanner.py` 查看任务分类，或手动查看 `input/task325.json`。
...
- `fusion_hint`: Study baseline: 39 nodes, ops=Add+And+Cast+Concat+Conv+Div+Equal+Less+Mul+Not+Or+ReduceSum+Slice+Sub
- `approach`: Conv + supporting ops (Reduce/Where). Refer to baseline for exact op sequence.

Baseline 实际架构: **Conv + logic gates** — Add+And+Cast+Concat+Conv+Div+Equal+Less+Mul+Not+Or+ReduceSum+Slice+Sub (39 nodes, 22 initializers, 5072 bytes)
...
baseline_ops: Add+And+Cast+Concat+Conv+Div+Equal+Less+Mul+Not+Or+ReduceSum+Slice+Sub
baseline_nodes: 39
baseline_size_bytes: 5072
```
