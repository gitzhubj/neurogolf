...
_NEUROGOLF_DIR_KAGGLE = "/kaggle/input/competitions/neurogolf-2026/"

def _get_data_dir():
    """Auto-detect data directory: local project path or Kaggle environment."""
    import os
    local = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "input")
    if os.path.isdir(local):
        return local
    return _NEUROGOLF_DIR_KAGGLE
_COLORS = [
    (0, 0, 0),
    (30, 147, 255),
    (250, 61, 49),
...
def load_examples(task_num):
  ...
  with open(os.path.join(data_dir, f"task{task_num:03d}.json")) as f:
